...
- **JointPRS-auto**: **No** tuning data. [Compute the Auto Version of JointPRS]
- **JointPRS**: **Exist** tuning data. [Compute the Meta Version and the Tune Version of JointPRS, and Utilize a Data-adaptive Approach to Select the Optimal Version]
...
**For further score combination**, it depends on the availability of the validation dataset:
- If there is **no** validation dataset, we use **JointPRS-auto**, and the genom-wide score from the previous step will be the final score for complex traits prediction.
- If there **exists** a validation dataset, we use **JointPRS** and **JointPRS-auto**. We recommend for each global shrinkage parameter `${param_phi}` from set **{1e-06, 1e-04, 1e-02, 1e+00, auto}**, calculate the polygenic score of the target population using each of the discovery population posterior SNP effect size estimates (four discovery populations in our example and four PRS scores in this step) and learn a linear combination of the polygenic scores as the final score using the validation dataset. Then select the optimal global shrinkage parameter in the validation dataset. We recommend standardize the polygenic scores (i.e., converting the scores to zero mean and unit variance) in both validation and testing datasets before linear combination.
